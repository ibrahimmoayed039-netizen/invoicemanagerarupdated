...
// مسح جميع البيانات (العملاء/الفواتير/التسديدات) — يُستدعى فقط بعد تأكيد المستخدم من الواجهة.
// يأخذ نسخة أمان صريحة تلقائياً قبل المسح (انظر db.clearAllData) فلا داعي لأي نسخ إضافي هنا.
ipcMain.handle('data:clearAll', async () => {
  const cleared = db.clearAllData();
  return { ok: true, settings: cleared.settings };
});
