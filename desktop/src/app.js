...
async function renderSettings(area) {
  ...
  // ---- منطقة الخطر ----
  area.appendChild(el('div', { class: 'section-title' }, ['⚠ منطقة الخطر']));
  area.appendChild(el('div', { class: 'field-hint' }, [
    'يمسح جميع العملاء والفواتير والتسديدات نهائياً، ويصفّر عدّاد أرقام الفواتير. لا يمسح اسم المنشأة/الشعار/الترخيص/إعدادات النسخ الاحتياطي. يؤخذ ملف نسخة أمان تلقائي بمجلد النسخ الاحتياطي قبل المسح مباشرة (باسم يبدأ بـ before-clear-all)، لكن الأفضل تصدير نسخة احتياطية يدوية بنفسك قبل المتابعة.',
  ]));
  const dangerActions = el('div', { class: 'toolbar', style: 'margin-top:8px' });
  dangerActions.appendChild(el('button', { class: 'btn btn-danger', onclick: () => openClearAllDataModal() }, ['🗑 مسح جميع البيانات']));
  area.appendChild(dangerActions);
}

// تأكيد مضاعف قبل مسح كل البيانات: يطلب من المستخدم كتابة كلمة "مسح" حرفياً لتفعيل زر التأكيد،
// لتقليل احتمال الضغط عليه بالخطأ نظراً لخطورة العملية وعدم إمكانية التراجع عنها من داخل البرنامج.
function openClearAllDataModal() {
  const confirmWord = 'مسح';
  const input = el('input', { class: 'input', placeholder: 'اكتب: ' + confirmWord, style: 'text-align:center;font-weight:700' });
  const body = el('div', {}, [
    el('p', {}, ['هل أنت متأكد؟ سيتم مسح جميع العملاء والفواتير والتسديدات نهائياً ولا يمكن التراجع عن هذا من داخل البرنامج.']),
    el('p', {}, ['للتأكيد، اكتب كلمة "' + confirmWord + '" بالمربع أدناه ثم اضغط "مسح كل شيء نهائياً".']),
    el('div', { class: 'field', style: 'margin-top:8px' }, [input]),
  ]);
  const footer = [
    el('button', { class: 'btn btn-danger', onclick: async () => {
      if (input.value.trim() !== confirmWord) {
        toast('اكتب كلمة "' + confirmWord + '" تماماً للتأكيد', true);
        return;
      }
      ...
      if (res.ok) {
        ...
        toast('تعذّر مسح البيانات', true);
      }
    }}, ['مسح كل شيء نهائياً']),
    el('button', { class: 'btn btn-ghost', onclick: closeModal }, ['إلغاء']),
  ];
  openModal('تأكيد مسح جميع البيانات', body, footer);
}
